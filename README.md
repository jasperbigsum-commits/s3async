...
正式版本使用 `v主版本.次版本.修订版本`，例如 `v1.2.3`；不使用 `v1.2` 或 `v01.2.3`。已发布版本不覆盖或移动 tag，修复后递增版本号。发布说明由 `scripts/generate-release-notes.py` 自动生成，包含产物清单、SHA256、与上一版本的提交对比和校验方法，无需手写。
...
s3async sync ./restore --download --config ./config.yaml --on-collision skip --async
```
...
本地不能创建名称为 `/` 的目录，因此下载会折叠这些目录段；如果多个对象映射到同一个本地路径，默认整个规划会报冲突而非覆盖（报错会同时列出两个完整 key），加 `--on-collision skip` 可跳过这些对象并继续下载其余文件。仅由斜杠组成或以斜杠结尾的目录标记仍跳过，不创建空目录。重新上传不会自动还原被折叠的斜杠。

### 路径风格：`path_style: faithful`（1:1 还原 key 结构）

S3 key 本质是扁平字符串，`backup//a.txt` 里 `//` 之间是一个**空目录段**，Linux 上不可能按原样建目录（空名和 `/` 都不能做文件名），默认 `collapse` 风格只能折叠它——这正是上面碰撞报错的来源。`faithful` 风格用可逆转义给每个 key 唯一的本地路径：

| S3 key 段 | 本地目录名 | 说明 |
| --- | --- | --- |
| ``（空段） | `%2F` | `backup//a.txt` 落盘为 `backup/%2F/a.txt` |
| `.` | `%2E` | 不再丢弃 |
| `..` | `%2E%2E` | 永不解析为上级目录，穿越攻击天然免疫 |
| 真实存在的 `%2F`/`%2E`/`%25` | `%` 转义为 `%25` | 如 `%2F` 存为 `%252F`，解码严格双射 |

```yaml
# config.yaml（必须写进配置文件：规划、执行、daemon 都从这里读，保证三端一致）
path_style: faithful
```

```bash
s3async validate --config ./config.yaml   # 确认 path_style: faithful
s3async sync ./restore --download --config ./config.yaml --async
```

- 上传自动把转义名解码拼回原始 key，来回一致；`--include/--exclude` 仍匹配原始相对 key。
- 其余 unsafe 检查原封不动：`..\` 反斜杠、`:`、NUL、Windows 保留名、目标内 symlink 照样拒绝；仅大小写不同的 key 在大小写不敏感文件系统上仍按 `--on-collision` 处理。
- 已有 collapse 落盘的老目录不受影响（默认不变）；切到 faithful 后是另一套目录树，两种风格不要混用同一个目标目录。
- 环境变量 `S3ASYNC_PATH_STYLE=faithful` 同等生效。
